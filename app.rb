...
post '/search' do
	...
	@results, @valid, @error_message = getResults(params["city"], departure, arrival)
...
def getResults(city, departure, arrival)
	...
	#request.body = '[,[[,"ad","[,[,[[,[\"'+city+'\"],,\"'+d_date+'\",,[,'+"#{d_hour}"+',1440],,,0],[,,[\"'+city+'\"],\"'+a_date+'\",,,[,'+"#{a_hour}"+',1260],,0]]],[,-52.895362126290784,176.578125,79.0360206117436,67.171875],2]","1663421298220535",37]],[,[[,"b_al","fs:3377"],[,"b_qu","0"],[,"b_qc","4"]]]]'
	...
	unless response.code != 200
		return [nil, false, "#{response.code} - #{response.message}"]
	end
